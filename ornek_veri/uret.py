...
def _denge_dogrula() -> None:
    toplam_borc = sum((s[2] for s in MIZAN_SATIRLARI), start=Decimal("0"))
    toplam_alacak = sum((s[3] for s in MIZAN_SATIRLARI), start=Decimal("0"))
    toplam_borc_bakiye = sum((s[4] for s in MIZAN_SATIRLARI), start=Decimal("0"))
    toplam_alacak_bakiye = sum((s[5] for s in MIZAN_SATIRLARI), start=Decimal("0"))
